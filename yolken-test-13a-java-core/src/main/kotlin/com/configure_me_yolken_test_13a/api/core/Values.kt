...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { YolkenTest13aInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw YolkenTest13aInvalidDataException("`$name` is not set")
            is JsonNull -> throw YolkenTest13aInvalidDataException("`$name` is null")
            else -> throw YolkenTest13aInvalidDataException("`$name` is invalid, received $this")
        }
